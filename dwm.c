...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (occ & 1 << i)
			drw_rect(drw, x + boxw, bh - boxw - 2, w - ( 2 * boxw + 1), boxw,
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		...
		XTextProperty dstp;
		dstp.value = (unsigned char *) "dwmbar";
		dstp.encoding = XA_STRING;
		dstp.format = 8;
		dstp.nitems = strlen((char *) dstp.value);
		XSetWMName(dpy, m->barwin, &dstp);
	}
}
